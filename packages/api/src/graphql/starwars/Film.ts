...
export const filmResolvers = {
  Query: {
    ...
    getFilm: (_: any, { id }: { id: number }) => getOne('starwars_films', id),
  },

  Film: {
    species: ({ species }: Film) => batchLoad(species as number[], 'starwars_species'),
    starships: ({ starships }: Film) => batchLoad(starships as number[], 'starwars_starships'),
    vehicles: ({ vehicles }: Film) => batchLoad(vehicles as number[], 'starwars_vehicles'),
    characters: ({ characters }: Film) => batchLoad(characters as number[], 'starwars_people'),
    planets: ({ planets }: Film) => batchLoad(planets as number[], 'starwars_planets'),
  },
}
